...
class Login(Interactor[LoginRequest, AuthResponse]):
    def __init__(self, users_repository: IUsersRepository, password_hasher: IPasswordHasher):
    ...
    async def __call__(self, request: LoginRequest) -> AuthResponse | None:
        ...
        if not user or not self.password_hasher.verify_password(request.password,
