...
from app.application.contracts.users.get_user_request import GetUserListRequest, GetUserRequest
from app.application.contracts.users.user_reponse import UserResponse, UserListResponse
from app.application.usecase.users.get_user import GetUserUseCase, GetUsersUseCase
...
@inject
async def get_users(
        request: Annotated[GetUserListRequest, Depends()],
...
@inject
async def get_user(
        request: Annotated[GetUserRequest, Depends()],
        interactor: FromDishka[GetUserUseCase]
) -> UserResponse:
